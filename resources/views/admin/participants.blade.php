...
<body class="bg-[#F4F5F7] min-h-screen flex flex-col font-sans">
    ...
    @php
        $tab            = $tab ?? 'peserta';
        $categoryOptions = ['Basic', 'Advanced', 'Basic-Advanced', 'Online', 'Workshop', 'Advanced-Workshop', 'Basic-Advanced + Workshop'];
                            ...
                            <tr class="hover:bg-gray-50/80 transition">
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        @if($tab === 'peserta' && $item->user?->wa_joined_groups && $item->user->hasJoinedWaGroup(\App\Models\WaGroupLink::normalizeCategory((string) $item->ticket_category)))
                                            <span title="Sudah join grup WhatsApp {{ \App\Models\WaGroupLink::normalizeCategory((string) $item->ticket_category) }}"
                                ...
                                <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                    ...
                                    @else
                                        ...
                                        <div class="flex items-center justify-center gap-1.5">
                                            ...
                                            @else
                                                @if($item->user)
                                                    @php $waGroup = \App\Models\WaGroupLink::normalizeCategory((string) $item->ticket_category); @endphp
                                                    @if($item->user->hasJoinedWaGroup($waGroup))
                                                        <form method="POST" action="{{ route('admin.participants.waToggle', $item->id) }}" onsubmit="return confirm('Kosongkan tanda WA untuk {{ addslashes($item->name_with_title ?: $item->full_name) }}?')">
                                                            @csrf
                                                            <button type="submit" title="Batalkan status sudah-join grup WA {{ $waGroup }}"
                                                    ...
                                                    @else
                                                        <form method="POST" action="{{ route('admin.participants.waToggle', $item->id) }}" onsubmit="return confirm('Tandai {{ addslashes($item->name_with_title ?: $item->full_name) }} sebagai SUDAH JOIN grup WhatsApp {{ $waGroup }}?')">
                                                            @csrf
                                                            <button type="submit" title="Konfirmasi manual: peserta sudah join grup {{ $waGroup }} tanpa klik tombol di website"
